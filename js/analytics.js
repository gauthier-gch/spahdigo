...
function renderAnalyticsPage() {
  page.innerHTML = `
    ...
    <div class="analytics-body">
      ...
      <div class="section-label" style="padding:8px 0;">Trier par critere</div>
      <div class="filter-bar" id="filter-criteria" style="overflow-x:auto;flex-wrap:wrap;padding-bottom:4px;gap:6px;">
        ...
        <button class="filter-chip" data-crit="gout_vin">Gout Vin</button>
        ...
        <button class="filter-chip" data-crit="viabilite_saisonniere">Viabilite Sais.</button>
  ...
  document.getElementById("group-select").addEventListener("change", e => {
    ...
    else document.getElementById("top-bars-list").innerHTML = `<p style="color:var(--muted);font-size:13px;">Selectionnez un groupe.</p>`;
  });
...
async function loadGroupOptions() {
  ...
  if (snap.empty) { select.innerHTML += `<option disabled>Aucun groupe</option>`; document.getElementById("top-bars-list").innerHTML=`<p style="color:var(--muted);font-size:13px;text-align:center;">Creez un groupe depuis l'onglet Social !</p>`; return; }
  ...
  document.getElementById("top-bars-list").innerHTML = `<p style="color:var(--muted);font-size:13px;">Selectionnez un groupe.</p>`;
}
...
async function loadTopBars() {
  ...
  if (!sorted.length) { listEl.innerHTML=currentFilter==="group"?`<p style="color:var(--muted);font-size:13px;text-align:center;">Aucun bar note par tous les membres.</p>`:`<p style="color:var(--muted);font-size:13px;text-align:center;">Aucune note trouvee.</p>`; return; }
